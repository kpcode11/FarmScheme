...
const SchemesList = () => {
  ...
  useEffect(() => {
    const fetchSchemes = async () => {
      try {
        setLoading(true);
        console.log(`Making request to: http://localhost:8001/api/v1/schemes?page=${currentPage}&limit=${limit}`);
        
        // Include pagination parameters in the request
        const response = await axios.get(`http://localhost:8001/api/v1/schemes?page=${currentPage}&limit=${limit}`);
        ...
        if (response.data && response.data.data && response.data.data.scheme) {
          ...
          setTotalSchemes(response.data.data.TotalSchemes || (response.data.data.TotalPages * limit));
        } else {
  ...
  console.log('Pagination state:', { currentPage, totalPages, totalSchemes, limit });

  if (loading) return <div className="loading">Loading schemes...</div>;
  if (error) return <div className="error">{error}</div>;
  if (schemes.length === 0 && !loading) return <div className="empty">No schemes available.</div>;
            ...
            <div className="scheme-card" key={scheme._id}>
              ...
              <div className="scheme-details">
                {scheme.schemeCategory && (
                  <p><strong>Category:</strong> {scheme.schemeCategory}</p>
                )}
                {scheme.level && (
                  <p><strong>Level:</strong> {scheme.level}</p>
                )}
                {scheme.details && (
                  <p><strong>Details:</strong> {scheme.details.substring(0, 200)}...</p>
                )}
              </div>
